...
void AnimationController::update(float deltatime){
    m_ticks++;
    m_current_frame = (m_ticks / m_pAnimation->getFps()) % m_pAnimation->m_sequenceList.size();
}
...
void PlayerController::draw(IGraphics *graphics){
    std::string text = "X: " + std::to_string(m_pOwner->getPosition().x) + " Y: " + std::to_string(m_pOwner->getPosition().x);
